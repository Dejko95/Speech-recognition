...
public class LPC {
	...
	public static double[] calcCoefficientVector(double[] samples, int offset, int N) { //offset - kraj prethodnog frejma
		double[] alpha = new double[p];													//N - duzina frejma
		double[] R = new double[p+1];
	
		for (int i=0; i<=p; i++) {
			R[i] = autocorelationFunc(samples, offset, N, i);
	...
	public static void gaus(double[][] mat, int n) {
		for (int k=1; k<=n; k++) {
			for (int j=1; j<=n+1; j++) mat[k][j] /= mat[k][k];	//set mat[k][k] = 1, and scale rest of the row
			for (int i=1; i<=n; i++) {
				if (i == k) continue;
